...
public:
	...
	bool read_seq_data(const std::string& data_file,
					   abcdl::algebra::MatSet& train_seq_data,
					   const std::string& label_file,
					   abcdl::algebra::MatSet& train_seq_label,
                       int limit = -1);
private:
	bool read_data(const std::string& data_file,
                   abcdl::algebra::MatSet& matset,
                   int limit = -1);
...
bool RNNHelper::read_data(const std::string& data_file,
	...
	while(getline(in_file, data)){
        ...
        abcdl::algebra::Mat mat(rows, _feature_dim);
        for(size_t i = 0; i != rows ; i++){
            mat.set_data(1, i, helper.str2int(line_data[i]));
        }
        matset.push_back(mat);
        if(limit > 0 && (int)matset.size() >= limit){
            break;
        }
	}
	in_file.close();
